    ...
    const runBuild = () => {
      ...
      const compileNext = () => {
        if (fileIndex < this.files.length) {
          ...
          setTimeout(
            () => {
              ...
              if (file.includes("canary_bypass")) {
                this.addLog(
                  `     <span style="color:#ffff00;">DEBUG:</span> Stack canary check is active. Value seems predictable based on initial input vector.`,
                  "#a9a9a9",
                  true,
                )
                this.addLog(
                  `     <span style="color:#ffff00;">DEBUG:</span> Function pointer 'action_ptr' found in struct. Potential overwrite target.`,
                  "#a9a9a9",
                  true,
                )
              }
              if (file.includes("iam_module")) {
                this.addLog(
                  `     <span style="color:orange;">WARNING:</span> User metadata tags are uninitialized. This may affect cloud pivot stage. Check IAM user tags.`,
                  "#a9a9a9",
                  true,
                )
                this.addLog(
                  `     <span style="color:#00ffff;">INFO:</span> Linking against policy descriptor: '********consoleElement-policy'. Ensure GetPolicy permissions.`,
                  "#a9a9a9",
                  true,
                )
              }
          ...
          setTimeout(() => {
            this.addLog(
              `\nPayload compiled successfully. Ready for deployment.`,
              "#ffff00",
            )
            this.addLog(
              `\nAttempting connection: nc xorfidential-dc33.hexnova.quest 8888.`,
              "#ffff00",
            )
      ...
      this.addLog(
        `Starting new build cycle for 'Xorfidential' payload at ${new Date().toLocaleTimeString()}`,
      )
      compileNext()
    }
    runBuild()
