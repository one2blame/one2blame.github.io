...
export default class Desktop {
    ...
    bringToFront(windowEl) {
        ...
        if (windowEl && !windowEl.classList.contains('minimized')) {
            ...
            const taskbarEntry = this.taskbarApps.querySelector(`.taskbar-entry[data-window-id="${windowEl.id}"]`);
            if (taskbarEntry) {
    ...
    handleMouseMove(e) {
        ...
        newX = Math.max(0, Math.min(newX, this.desktop.clientWidth - this.dragTarget.clientWidth));
        newY = Math.max(0, Math.min(newY, this.desktop.clientHeight - this.dragTarget.clientHeight - taskbarHeight));
    ...
    handleWindowControls(e) {
        ...
        if (e.target.matches('.minimize-btn')) {
            ...
            const taskbarEntry = this.taskbarApps.querySelector(`.taskbar-entry[data-window-id="${targetWindow.id}"]`);
