export default class Ssh {
    ...
    start() {
        const runSession = () => {
            const target = this.websites[Math.floor(Math.random() * this.websites.length)];
            const steps = [
                `ssh root@${target} -p 22`,
                `Connecting to ${target}...`,
                `Connection established.`,
                `Authenticating with private key "id_rsa"...`,
                (Math.random() > 0.4) ? `Permission denied (private key).` : `Authentication successful.`,
            ...
            const processNextStep = () => {
                if (currentStep < steps.length) {
                    this.typeLine(steps[currentStep], () => {
                        ...
                        const delay = (steps[currentStep-1].includes("Retrying")) ? 3000 : Math.random() * 400 + 100;
