export default class H3xCLI {
                ...
                commands: {
                    "start": (game) => {
                        if (game.gameProgress === 0) {
                            ...
                            game.gameProgress = 1;
                            setTimeout(() => {
                                game.writeToTerminal("Scan complete. Vulnerable port found on 192.168.1.100.");
                                game.writeToTerminal("You can now 'scan 192.168.1.100' or 'login 192.168.1.100'.");
                            }, 1500);
                        } else {
                            game.writeToTerminal("Mission already in progress.");
                        }
                    },
                    "help": (game) => game.writeToTerminal("Available commands: start, help, status"),
                }
            },
            1: {
                commands: {
                    "scan": (game, target) => {
                        if (target === "192.168.1.100") {
                            ...
                            setTimeout(() => {
                                game.writeToTerminal("Port 22 (SSH) open. Weak password found: 'admin123'.");
                            }, 1500);
                        } else {
                    ...
                    "login": (game, target) => {
                        if (target === "192.168.1.100") {
                            ...
                            setTimeout(() => {
                                game.writeToTerminal('<span style="color:#00ffff;">Login successful! Welcome to Orion Corp Intranet.</span>', { isHtml: true });
                                game.systemAccess = true;
                                game.gameProgress = 2;
                                game.writeToTerminal("You can now 'ls' (list files) or 'cd' (change directory).");
                            }, 2000);
                        } else {
                            game.writeToTerminal("Invalid target for login.");
                        }
                    },
                    "help": (game) => game.writeToTerminal("Available commands: scan [ip], login [ip], status, help"),
                }
            },
            2: {
                commands: {
                    "ls": (game, dir) => {
                        if (game.currentDir === "/") {
                            if (!dir || dir === ".") game.writeToTerminal("Files: security_log.txt, /data");
                            else game.writeToTerminal("ls: directory not found.");
                        } else if (game.currentDir === "/data") {
                            if (!dir || dir === ".") game.writeToTerminal("Files: financial_records.db, Project_Chimera.zip");
                            else game.writeToTerminal("ls: directory not found.");
                        }
                    },
                    "cd": (game, dir) => {
                        if (dir === "data" || dir === "/data") {
                            ...
                            game.writeToTerminal("Now you can 'download Project_Chimera.zip'.");
                            game.gameProgress = 3;
                        } else if (dir === "/" || dir === "..") {
                            game.currentDir = "/";
                    ...
                    "download": (game, file) => {
                        if (file === "Project_Chimera.zip" && game.gameProgress === 3) {
                            ...
                            setTimeout(() => {
                                game.writeToTerminal('<span style="color:#00ff00;">Download complete.</span>', { isHtml: true });
                                game.inventory.push("Project_Chimera.zip");
                                game.endGame();
                            }, 2500);
                        } else {
                            game.writeToTerminal("download: file not found or not in current directory.");
                        }
                    },
                    "help": (game) => game.writeToTerminal("Available commands: ls, cd [directory], download [file], status, help"),
                }
            }
        };
    }
    ...
    initDOM() {
        ...
        this.endMessageContainer = this.consoleElement.querySelector('.end-message-container');
    ...
    bindEvents() {
        ...
        this.consoleElement.addEventListener('click', () => { /* The keydown listener on document handles focus logic */ });
    }
    ...
    processCommand(commandLine) {
        this.writeToTerminal(`<span class="prompt">agent@orion:~$</span> ${commandLine}`, { isHtml: true });
        
        const parts = commandLine.trim().toLowerCase().split(/\s+/);
        const command = parts[0];
        const args = parts.slice(1);

        const currentState = this.gameStates[this.gameProgress] || this.gameStates[2];
    ...
    showStatus() {
        ...
        this.writeToTerminal(`Inventory: ${this.inventory.length > 0 ? this.inventory.join(', ') : 'Empty'}`);
        this.writeToTerminal(`System Access: ${this.systemAccess ? '<span style="color:#00ff00;">Granted</span>' : '<span style="color:red;">Denied</span>'}`, { isHtml: true });
    }
